...
Our framework is shown in
Figure~\ref{fig:flowchart}.

\section{Approach}
We show our framework in Figure~\ref{fig:flowchart}.  Specifically, like playing a Twenty Questions game, at each step the policy evaluates the current state to decide an action that either reject the occurrence of the query, pose a question about a context class, or stop and run the query detector. If it poses a question about a context class, it will take the action to run context classifiers/detectors and observe responses, then update the states by refining the search area for the query object using spatially-aware contextual models. Finally, the query object detector will be evaluated in the predicted search area if the policy thinks enough contextual information has been gathered and decides to stop. In this section, we will first present the imitation learning of the 20 questions policy, then introduce how our approach predicts the search area for the query given observations of context classes from responses of previous asked questions.

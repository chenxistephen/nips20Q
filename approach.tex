...
\caption{(i) Weighted vote map from sky to boat.}
\label{fig:vote_sky_boat}
\end{subfigure}
\begin{subfigure}[t]{0.48\textwidth}
\centering
\includegraphics[height=1.2in]{figures/votemap.pdf}
\caption{(ii) Example context vote maps.}
\label{fig:votemap}
\end{subfigure}
\caption{(i):The first row shows example training pairs of the sky and the boat. 
The second row shows the test image and the weighted voting map. 
The width of the arrow denotes the weighted
similarity $W(s_{c_t}^i,s;\theta^W)$ between the test segment
(highlighted in yellow) and one of the training sky segments (highlighted
in light blue).
(ii) Each pair shows the original image and the voted probability
